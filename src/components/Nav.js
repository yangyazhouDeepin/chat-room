...
class Nav extends Component {
    ...
    if (!params) {
      ...
      if (query.pid) {
        ...
        attr = {
          ...
          Content: JSON.stringify({platUserId: query.platUserId, token: query.token})
        }
      }
      api(attr)
      .then(res => {
  ...
  initSocket = () => {
    ...
    this.socket = new Socket({
      ...
      receive: (data) => {
        console.log(data)
        let friend = null
        if (data.code < 0 && data.message) message.error('服务器异常，请联系管理员')
        switch(data.code) {
          // 登录
          case 10002:
          ...
          case 10013:
            if (data.data.id === this.selfId) {
              if (data.data.music === 1) {
                message.success('设置开启声音提醒成功')
              } else if (data.data.music === 0) {
                message.success('设置关闭声音提醒成功')
              } else {
                message.success('设置个人资料成功')
  ...
  getNavList() {
    let temp = this.props.navList.slice(0, 1)
    if (this.props.user.userType !== undefined && this.props.user.userType !== 0) {
      temp = this.props.navList
    }
    return temp.map((nav, idx) => {
      let hasMark = false
      let count;
      if (idx === 0) {
        count = 0
        for(let key in this.props.list) {
          if (this.props.list[key].count) count += this.props.list[key].count
          if (count) hasMark = true
        }
      }
      if (idx === 2 && this.props.user.recommendCount > 0) {
        hasMark = true;
        count = this.props.user.recommendCount
      } else if (idx === 3 && this.props.user.systemCount > 0) {
        hasMark = true
        count = this.props.user.systemCount
      }
      ...
      return (
        <NavLink
          ...
          to={nav.link}
        >
          { hasMark ? <span className="new-message-mark">{ count }</span> : null}
          {nav.title}
        </NavLink>
      )
    })
